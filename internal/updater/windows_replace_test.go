...
func TestReplaceWindows(t *testing.T) {
	...
	validArchive := createDummyZip(t, "bva.exe", dummyContent)
	emptyArchive := createDummyZip(t, "notbva.exe", dummyContent)
	...
	tests := []struct {
		...
		{
			name:        "missing_bva_win",
			body:        bytes.NewBuffer(emptyArchive),
			wantErr:     true,
			errContains: "bva.exe not found in archive",
		},
		{
		...
		},
		{
			...
			wantErr:     true,
			errContains: "failed to open bva.exe in zip",
		},
		{
			name: "close_error",
			body: bytes.NewBuffer(validArchive),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := replaceWindows(tt.body, "/test/bva.exe")
...
func TestDefaultZipOpen(t *testing.T) {
	validArchive := createDummyZip(t, "bva.exe", "content")

...
func TestReplacePOSIX(t *testing.T) {
	...
	validArchive := createDummyTarGz(t, "bva", dummyContent)
	...
	emptyArchive := createDummyTarGz(t, "notbva", dummyContent)
	...
	tests := []struct {
		...
		{
			name:        "missing_bva_posix",
			body:        bytes.NewBuffer(emptyArchive),
			wantErr:     true,
			errContains: "bva executable not found in archive",
		},
		{
		...
		{
			name: "chmod_error",
			body: bytes.NewBuffer(createDummyTarGz(t, "bva", dummyContent)),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := replacePOSIX(tt.body, "/test/bva")
...
func TestReplacePOSIXFull(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := replacePOSIX(tt.body, filepath.Join(tempDir, "bva"))
			if (err != nil) != tt.wantErr {
				t.Errorf("replacePOSIX() error = %v, wantErr %v", err, tt.wantErr)
			}
			if err != nil && tt.errContains != "" {
...
func TestDefaultTempFileClose(t *testing.T) {
	tempFile, err := os.CreateTemp("", "bva-test-close-*")
	if err != nil {

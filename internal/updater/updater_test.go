...
func TestUpdate(t *testing.T) {
	...
	validArchivePOSIX := createDummyTarGz(t, "bva", dummyContent)
	validArchiveWin := createDummyZip(t, "bva.exe", dummyContent)
	...
	validJSON := fmt.Sprintf(`{
		"tag_name": "v1.1.0",
		"assets": [
			{"name": "blind-vault-audit-linux-amd64.tar.gz", "browser_download_url": "http://dl/linux-amd64", "digest": "%s"},
			{"name": "blind-vault-audit-linux-arm64.tar.gz", "browser_download_url": "http://dl/linux-arm64", "digest": "%s"},
			{"name": "blind-vault-audit-macos-intel.tar.gz", "browser_download_url": "http://dl/macos-intel", "digest": "%s"},
			{"name": "blind-vault-audit-macos-apple-silicon.tar.gz", "browser_download_url": "http://dl/macos-silicon", "digest": "%s"},
			{"name": "blind-vault-audit-windows-x64.zip", "browser_download_url": "http://dl/windows-x64", "digest": "%s"},
			{"name": "blind-vault-audit-windows-arm64.zip", "browser_download_url": "http://dl/windows-arm64", "digest": "%s"}
		]
	...
	tests := []updateTestCase{
		...
		},
		{
			...
			mockOS: osWindows,
			mockExecutable: func() (string, error) {
				return filepath.Join(t.TempDir(), "bva.exe"), nil
			},
		...
		},
		{
			...
			mockOS: "linux",
			mockExecutable: func() (string, error) {
				return filepath.Join(t.TempDir(), "bva"), nil
			},
			mockChmod:  func(_ string, _ os.FileMode) error { return nil },
			mockRename: func(_, _ string) error { return nil },
			mockRemove: func(_ string) error { return nil },
